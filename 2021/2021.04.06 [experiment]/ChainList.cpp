...
template <class TT> Chain<TT>::Chain() {
  m_first_node = nullptr;
  m_current_node = nullptr;
  m_size = 0;
};

template <class TT> void Chain<TT>::checkIndex(int index) {
...
template <class TT> int Chain<TT>::index(TT &element) {
...
template <class TT> TT &Chain<TT>::get(int index) {
  checkIndex(index);
  Node<TT> *node_traverse = m_first_node;
...
template <class TT> void Chain<TT>::insert(int index, TT &element) {
  if (index < 0 || index > m_size)
