...
void changeArrayLength(TT*& old_array, int old_length, int new_length) {
    // auto increment
    if (new_length < 0) {
        throw runtime_error("new length of array must more than 0\n");
    }
...
template <class TT>
OrderList<TT>::OrderList(int init_len) {
    if (init_len < 0) {
        throw runtime_error("init length of array must more than 0\n");
    }
...
template <class TT>
void OrderList<TT>::checkIndexExist(int index) {
    // check index if valid, else throw error
    if (index < 0 || index > m_listSize) {
        ostringstream s;
        s << "index = " << index << "size = " << m_listSize << endl;
        throw runtime_error(s.str());
    };
};

template <class TT>
void OrderList<TT>::insert(int index, const TT& element) {
...
template <class TT>
void OrderList<TT>::erase(int index) {
    checkIndexExist(index);
...
int main() {
    ...
    try {
        linear_list.erase(4);  // test: delete index of 4, and test_index_5 will
                               // in the position 4
    } catch (const runtime_error& exc) {
        cout << exc.what();
    }
